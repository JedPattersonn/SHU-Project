...
async function validateAndInsertData(rows: CsvRow[], year: number) {
  ...
  for (const row of rows) {
    ...
    const annualConsumeLowTarifPerc = parseFloat(row.annual_consume_lowtarif_perc);
    const smartmeterPerc = parseFloat(row.smartmeter_perc);

    if (!row.purchase_area || !row.street || !row.zipcode_from || !row.zipcode_to || !row.type_of_connection) {
      console.error(`Skipping row due to missing required fields: ${JSON.stringify(row)}`);
      continue;
    }

    if (isNaN(numConnections) || isNaN(deliveryPerc) || isNaN(percOfActiveConnections) || 
        isNaN(typeConnPerc) || isNaN(annualConsume) || isNaN(annualConsumeLowTarifPerc) || 
        isNaN(smartmeterPerc)) {
      console.error(`Skipping row due to invalid numeric values: ${JSON.stringify(row)}`);
      continue;
    }
