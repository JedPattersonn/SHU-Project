...
export async function getCoordinatesForPostalCode(
  ...
  try {
    ...
    const match = data.response.docs[0].centroide_ll.match(
      /POINT\(([-\d.]+) ([-\d.]+)\)/
    );
    ...
    console.warn(
      `Using fallback coordinates for postal code ${postalCode}:`,
      error
    );
    return getFallbackCoordinates(postalCode);
  }
...
export async function enrichEnergyDataWithCoordinates(
  data: (typeof energyData.$inferSelect)[]
): Promise<EnergyDataWithCoordinates[]> {
  const postalCodeGroups = new Map<
    string,
    (typeof energyData.$inferSelect)[]
  >();
  ...
  const batchSize = 500;
  const batches = Array.from(postalCodeGroups.entries()).reduce(
    (acc, _, i) => {
